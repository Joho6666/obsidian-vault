...
本项目将知识管理分为三层：
- **原始资料层**（`raw 原始资料/`）：不可变的事实来源
- **知识层**（`wiki 知识层/`）：LLM 维护的结构化知识网络
...
![Pasted image 20260410195644.png|697](Attachments 附件/Pasted%20image%2020260410195644.png)

系统通过三层架构实现知识的持续积累和复利增长。

## 🏗️ Architecture · 三种文件类型

![Pasted image 20260410195715.png](Attachments 附件/Pasted%20image%2020260410195715.png)
...
![Pasted image 20260410195804.png](Attachments 附件/Pasted%20image%2020260410195804.png)

### 1. Ingest（导入）
添加新资料到 `raw 原始资料/`，LLM 阅读并：
- 创建来源摘要页
- 更新相关实体/概念页
- 维护交叉引用
- 记录到操作日志
...
![Pasted image 20260410195828.png](Attachments 附件/Pasted%20image%2020260410195828.png)
...
![Pasted image 20260410195907.png](Attachments 附件/Pasted%20image%2020260410195907.png)

1. **保持原始资料不变**：`raw 原始资料/` 目录只添加不修改
...
5. **开始使用**
   ```
   # 导入新资料
   "请基于 raw 原始资料/xxx.pdf 进行 Ingest"
...
| 类型 | 路径 | 用途 |
|------|------|------|
| Source Summary | `wiki 知识层/sources 来源摘要/` | 单个来源的摘要和要点 |
| Entity Page | `wiki 知识层/entities 实体页/` | 人物、书籍、项目等实体 |
| Concept Page | `wiki 知识层/concepts 概念页/` | 方法、理论、模型等概念 |
| Comparison | `wiki 知识层/comparisons 比较分析/` | 对比分析 |
| Overview | `wiki 知识层/overview 总览综合/` | 主题综述和总览 |
...
<img src="Attachments 附件/539.jpg" width="224" alt="539.jpg">
